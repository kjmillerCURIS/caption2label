...
def submit_patch_alignment_multilabel_prevalent_classes_only_eval_runs():
            ...
            for embedder_only in [False, True]:
                for simple_avg in [False, True]:
                    for drop_clf in [True]:
                        ...
                        job_name = 'eval_clip_with_patch_alignment_and_multilabel_prevalent_classes_only' + s_or_empty(IS_LACLIP, '_laclip') + snake_suffix
                        if JOB_NAME_FILTER is not None:
                            if job_name not in JOB_NAME_FILTER:
                                continue

                        my_cmd = 'qsub -N %s -v EXPERIMENT_DIR=%s,IS_LACLIP=%d run_eval_patch_alignment_with_multilabel_prevalent_classes_only_generic.sh'%(job_name, experiment_dir, int(IS_LACLIP))
...
if __name__ == '__main__':
    submit_patch_alignment_multilabel_prevalent_classes_only_eval_runs()
