...
def submit_visualize_training_examples_runs():
            ...
            for embedder_only in [False, True]:
                for simple_avg in [False, True]:
                    for drop_clf in [True]:
                        ...
                        job_name = 'visualize_training_examples' + s_or_empty(IS_LACLIP, '_laclip') + snake_suffix
                        if JOB_NAME_FILTER is not None:
                            if job_name not in JOB_NAME_FILTER:
                                continue

                        my_cmd = 'qsub -N %s -v EXPERIMENT_DIR=%s,IS_LACLIP=%d run_visualize_training_examples_generic.sh'%(job_name, experiment_dir, int(IS_LACLIP))
                        print('submitting vis run: "%s"'%(my_cmd))
                        os.system(my_cmd)
                        if DEBUG:
...
if __name__ == '__main__':
    submit_visualize_training_examples_runs()
